...
physical network devices. This demo uses a custom UI to visualise the network
topology to help illustrate use cases such as device migration and service
repair.
...
tenants can be expanded to show their VPN and data-centre endpoints.
...
## Topology
The topology is split into different domains and each device belongs to a
domain. The default domains are:

- **Data Center:** This has the data-centre switching fabric and DCI devices.
- **Transport:** This has the core and PE devices (the core devices are not
  configured as part of the demo).
...
data-centre endpoints. These can be seen by clicking a tenant name. Each
endpoint can also be expanded by clicking on it. From the sidebar, tenants and
endpoints can be created and deleted using the `+` and `x` buttons
respectively. The values displayed in the sidebar cannot be edited directly,
instead the `...` buttons provide shortcuts to the NSO *Configuration Editor*
to make changes.
