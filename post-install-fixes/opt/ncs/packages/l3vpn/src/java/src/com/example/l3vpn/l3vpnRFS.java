...
public class l3vpnRFS {
    ...
    public Properties create(ServiceContext context,
        ...
        NavuContainer topology = ncsRoot.getParent().
                container("http://example.com/tme-demo").
                container("topologies").list("topology").elem("demo");
        for(NavuContainer endpoint : endpoints.elements()) {
            try {
                ...
                NavuContainer conn = getConnection(topology,
                                               ...
                                               valueAsString());
                Endpoint peEndpoint = getConnectedEndpoint(
                                                conn,ceName);
                Endpoint ceEndpoint = getMyEndpoint(
                                                conn,ceName);
                ...
                vpnVar.putQuoted("PE", peEndpoint.device);
                ...
                vpnVar.putQuoted("LINK_PE_ADR",
                                 getIPAddress(peEndpoint.ipAddress));
                vpnVar.putQuoted("LINK_CE_ADR",
                                 getIPAddress(ceEndpoint.ipAddress));
                vpnVar.putQuoted("LINK_MASK",
                                 getNetMask(ceEndpoint.ipAddress));
                vpnVar.putQuoted("LINK_PREFIX",
                                 getIPPrefix(ceEndpoint.ipAddress));
                vpnVar.putQuoted("PE_INT_NAME", peEndpoint.interfaceName);
                vpnVar.putQuoted("CE_INT_NAME", ceEndpoint.interfaceName);
                ...
                if (service.container("qos").leaf("qos-policy").exists()) {
                    ...
                    qosVar.putQuoted("CE_INT_NAME",
                            ceEndpoint.interfaceName);
                    qosVar.putQuoted("PE_INT_NAME",
                            peEndpoint.interfaceName);
                    qosVar.putQuoted("VLAN_ID", vlan.valueAsString());
                    qosVar.putQuoted("PE", peEndpoint.device);
    ...
    private static class Endpoint {
        ...
        Endpoint(NavuContainer conn, String endpoint)
                throws NavuException {
            this.device = conn.leaf(endpoint + "-device").valueAsString();
            this.interfaceName = conn.container(endpoint + "-interface").
                    leaf("name").valueAsString();
            this.ipAddress = conn.container(endpoint + "-interface").
                    leaf("ip-address").valueAsString();
        }
    }

    private Endpoint getConnectedEndpoint(NavuContainer conn,
                                    String deviceName)
            throws NavuException {
        if (deviceName.equals(conn.leaf("a-end-device").valueAsString())) {
                return new Endpoint(conn, "z-end");
            }
            else {
                return new Endpoint(conn, "a-end");
            }
    }

    private Endpoint getMyEndpoint(NavuContainer conn,
            String deviceName)
                    throws NavuException {
        if (deviceName.equals(conn.leaf("a-end-device").valueAsString())) {
            return new Endpoint(conn, "a-end");
    ...
    private NavuContainer getConnection(NavuContainer topology,
        ...
        NavuList connections = topology.container("links").list("link");
        for(NavuContainer conn : connections.elements()) {
            if (deviceName.equals(conn.leaf("a-end-device").valueAsString()) ||
                deviceName.equals(conn.leaf("z-end-device").valueAsString())) {
                return conn;
            }
        }
        return null;
